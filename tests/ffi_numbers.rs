...
fn get_i64_basic() {
    ...
    let mut v: i64 = 0;
    let p = b"a";
    let rc = unsafe { qjson_get_i64(d, p.as_ptr() as *const c_char, p.len(), &mut v) };
    assert_eq!(rc, 0);
    assert_eq!(v, 42);
    unsafe { qjson_free(d) };
}
...
fn get_f64_basic() {
    ...
    let mut v: f64 = 0.0;
    let p = b"a";
    let rc = unsafe { qjson_get_f64(d, p.as_ptr() as *const c_char, p.len(), &mut v) };
    assert_eq!(rc, 0);
...
fn get_bool() {
    ...
    let mut v: c_int = -1;
    let p = b"a";
    let rc = unsafe { qjson_get_bool(d, p.as_ptr() as *const c_char, p.len(), &mut v) };
    assert_eq!(rc, 0);
    assert_ne!(v, 0);
    let p = b"b";
    let rc = unsafe { qjson_get_bool(d, p.as_ptr() as *const c_char, p.len(), &mut v) };
...
fn get_u64_max() {
    ...
    let mut v: u64 = 0;
    let p = b"a";
    let rc = unsafe { qjson_get_u64(d, p.as_ptr() as *const c_char, p.len(), &mut v) };
    assert_eq!(rc, 0);
    assert_eq!(v, u64::MAX);
    unsafe { qjson_free(d) };
}
...
fn cursor_get_u64_max() {
    ...
    let p = b"root";
    let rc = unsafe { qjson_open(d, p.as_ptr() as *const c_char, p.len(), &mut cur) };
    assert_eq!(rc, 0);
    let mut v: u64 = 0;
    let p = b"a";
    let rc = unsafe { qjson_cursor_get_u64(&cur, p.as_ptr() as *const c_char, p.len(), &mut v) };
    assert_eq!(rc, 0);
    assert_eq!(v, u64::MAX);
    unsafe { qjson_free(d) };
}
