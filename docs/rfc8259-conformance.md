...
|---|---|---|
| `i_number_huge_exp` | REJECT (`QJSON_NUMBER_OUT_OF_RANGE`) | f64 overflow surfaces at decode. |
| `i_number_very_big_negative_int` | varies — see below | ABNF-valid; representational, not structural. |
| `i_string_*` (UTF-16 surrogate halves in `\u` escapes) | REJECT (`QJSON_DECODE_FAILED`) | We require well-formed surrogate pairs. |
